...
[ "$("$SMOKE_ROOT/bin/dotfiles" theme current --target "$SMOKE_HOME")" = terminal-pro ]
grep -q "background[[:space:]]*#000000" "$SMOKE_HOME/.config/dotfiles/theme/kitty.conf"
SMOKE_DARK_TARGET=$(readlink "$SMOKE_HOME/.config/dotfiles/theme")
SMOKE_LIGHT_TARGET=$(find "$SMOKE_ROOT/themes/generated" -maxdepth 1 -type d -name 'terminal-basic-*' -print | sed -n '1p')
if command -v nvim >/dev/null 2>&1; then
    env \
        HOME="$SMOKE_HOME" \
